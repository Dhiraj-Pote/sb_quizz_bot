// Canto Structure - organized by Canto
const CANTOS = {
  1: {
    id: 1,
    name: 'Creation',
...
function getCanto(cantoId) {
  return CANTOS[cantoId] || null;
}

function getActiveCanto() {
  return Object.values(CANTOS).filter(canto => canto.active);
}

function getChaptersForCanto(cantoId) {
  const canto = CANTOS[cantoId];
  if (!canto) return [];
  
  // Return all quizzes for this canto's chapters
  return canto.chapters.map(chapterNum => {
    const quizId = `quiz_${cantoId}_${chapterNum}`;
